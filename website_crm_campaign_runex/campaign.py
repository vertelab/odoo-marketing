...
class website_sale(website_sale):
    ...
    def campaign_shop(self, page=1, category=None, campaign=None, search='', **post):
        ...
        return request.website.render("website_sale.products", {
            ...
            'pager': pager,
            'pricelist': pricelist,
            'products': campaign.product_ids.sorted(key=lambda r: r.campaign_sequence),
            'bins': table_compute().process(campaign.product_ids.sorted(key=lambda r: r.campaign_sequence), ppg),
            'rows': PPR,
            'styles': styles,
            'categories': categs,
